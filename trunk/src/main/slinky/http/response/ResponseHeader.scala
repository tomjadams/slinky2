...
object ResponseHeader {
  ...
  object Entity {
    ...
    def unapply(h: ResponseHeader) = h match {
      case slinky.http.response.Entity(x) => Some(x)
      case _ => None
    }
  }
  ...
  object General {
     ...
     */
    def unapply(h: ResponseHeader) = h match {
      case slinky.http.response.General(x) => Some(x)
      case _ => None
    }
  }
   ...
   */
  implicit def entity(eh: EntityHeader): ResponseHeader = slinky.http.response.Entity(eh)

  /**
   * Converts the given general header into a response header.
   */
  implicit def general(gh: GeneralHeader): ResponseHeader = slinky.http.response.General(gh)
